...
(function($){
	...
	Redactor.prototype = {
		...
		buildToolbar: function()
		{
			...
			$.each(RTOOLBAR[this.opts.toolbar], $.proxy(function(key,s)
			{
				...
				if (key == 'backcolor' || key == 'fontcolor' || typeof(s.dropdown) != 'undefined')
				{
					...
					else if (key == 'special_chars') this.buildSpecialChars(dropdown, key);
		...
		buildSpecialChars: function(dropdown, key)
		{	
			$(dropdown).width(250);

			var len = this.opts.special_chars_map.length;
			for (var i = 0; i < len; ++i)
			{
				var special_char = this.opts.special_chars_map[i];

				var swatch = $('<a rel="' + special_char[3] + '" href="javascript:void(null);" class="redactor_special_char_link">' + special_char[0] + '</a>').css({ 'font-size': '13px'});
				$(dropdown).append(swatch);
				
				var _self = this;
				swatch.click(function() { _self.execCommand('insertHtml', $(this).html()); return false; });
			}
			
			var cancel = "<br /><a href='javascript:void(null);' class='redactor_btn_dropdown_close' id='redactor_btn_modal_close'>" + RLANG.close + "</a>";
